...
const AuthPage: React.FC = () => {
  ...
  const handleEmailVerification = (e: React.FormEvent) => {
    ...
    if (!validateEmail(email)) {
      setError("Please use a valid academic email address (e.g., .edu or .ac.in).");
      return;
    }
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (user1.email !== email) {
        throw new Error("The authenticated email does not match the provided academic email.");
      }
          ...
          >
            {loading1 ? (
              ...
              <h1 className="flex text-xl justify-center gap-2 items-center">
                <img src="google.png" alt="google" className="h-10 bg-white rounded-full p-1" />
                Sign In with Google
              </h1>
            )}
          </button>
